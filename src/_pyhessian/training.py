...
parser.add_argument("--epochs", type=int, default=180, metavar="N", help="number of epochs to train (default: 10)")
...
parser.add_argument("--residual", action="store_false", help="do we need residula connect or not")

parser.add_argument("--cuda", action="store_false", help="do we use gpu or not")
parser.add_argument("--saving-folder", type=str, default="checkpoints/", help="choose saving name")
...
model = resnet(num_classes=10, depth=20, residual_not=args.residual, batch_norm_not=args.batch_norm)
if args.cuda:
    model = model.cuda()
...
for epoch in range(1, args.epochs + 1):
    ...
    with tqdm(total=len(train_loader.dataset)) as progressbar:
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            progressbar.set_postfix(loss=train_loss / total_num, acc=100.0 * correct / total_num)
